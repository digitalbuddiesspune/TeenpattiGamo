...
const FLIPPER_FACE_STYLE = {
  background: "linear-gradient(180deg, #1a5fa8 0%, #0a2f5c 48%, #041828 100%)",
  borderColor: "#4db8ff",
  color: "#ffffff",
};

export default function PlayingCard({ card, revealed, compact = false, className = "", style, centerSuitOnly = false }) {
  const hidden = !revealed || card.hidden;
  const suitSymbol = suitMap[card.suit] || "♠";
  const sizeClasses = compact
    ? "playing-card compact h-[52px] w-[36px] rounded-[6px] sm:h-[60px] sm:w-[42px]"
    : "playing-card h-[84px] w-[58px] rounded-[10px] sm:h-[96px] sm:w-[66px]";
  const rankClass = compact ? "text-[8px] leading-none sm:text-[9px]" : "text-sm leading-none sm:text-base";
  const cornerSuitClass = compact ? "text-[8px] leading-none sm:text-[9px]" : "text-base leading-none";
  const suitClass = compact ? "text-[1.05rem] sm:text-[1.2rem]" : "text-[2rem] sm:text-[2.35rem]";
  ...
  const accentClass = isFlipperCard
    ? "playing-card--flipper"
    : isJokerCard
      ? "playing-card--joker"
      : isReserveCard
        ? "playing-card--reserve"
        : "";
  ...
  const mergedStyle = isFlipperCard && !hidden
    ? { ...FLIPPER_FACE_STYLE, ...style }
    : style;
  ...
  if (hidden) {
    ...
    if (isFlipperCard) {
      return (
        <div
          className={[
            "hidden-playing-card hidden-playing-card--flipper relative overflow-hidden border border-[#4db8ff]/80 drop-shadow-[0_12px_18px_rgba(0,0,0,0.28)]",
            sizeClasses,
            className
          ].join(" ")}
          style={style}
        >
          <span className="absolute inset-0 bg-[linear-gradient(145deg,#1a5fa8_0%,#0a2f5c_42%,#041828_100%)]" />
          <span className="absolute inset-[2px] rounded-[inherit] border border-white/20 bg-[radial-gradient(circle_at_top,rgba(120,190,255,0.35),transparent_32%),linear-gradient(160deg,rgba(40,110,180,0.94)_0%,rgba(10,50,100,0.96)_52%,rgba(4,24,40,0.98)_100%)]" />
          <span
            className="absolute inset-[10%] rounded-[inherit] opacity-40"
            style={{
              backgroundImage:
                "repeating-linear-gradient(45deg, rgba(10,40,80,0.5) 0 6px, rgba(180,220,255,0.06) 6px 12px)",
            }}
          />
          <span className="absolute inset-[18%] rounded-[inherit] border border-[#7ec8ff]/40 bg-[linear-gradient(135deg,rgba(80,150,220,0.4),rgba(20,70,130,0.25))] shadow-[inset_0_0_0_1px_rgba(10,40,80,0.3)]" />
          <span className="absolute left-1/2 top-1/2 h-[26%] w-[26%] -translate-x-1/2 -translate-y-1/2 rotate-45 rounded-[4px] border border-[#9ad4ff]/45 bg-[linear-gradient(135deg,rgba(120,190,255,0.85),rgba(30,90,160,0.75))]" />
        </div>
      );
    }
  ...
  if (compact) {
    return (
      <div
        ...
        style={mergedStyle}
      >
        <div className={`playing-card__corner absolute left-[3px] top-[3px] flex flex-col font-black ${inkClass}`}>
          <span className={rankClass}>{card.rank}</span>
          {centerSuitOnly ? null : <span className={cornerSuitClass}>{suitSymbol}</span>}
        </div>
        <div className={`absolute inset-0 flex items-center justify-center font-black leading-none ${inkClass} ${suitClass}`}>
          {suitSymbol}
        </div>
        <div
          className={`playing-card__corner absolute bottom-[3px] right-[3px] flex rotate-180 flex-col font-black ${inkClass}`}
        >
          <span className={rankClass}>{card.rank}</span>
          {centerSuitOnly ? null : <span className={cornerSuitClass}>{suitSymbol}</span>}
        </div>
  ...
  return (
    <div
      className={[
        `relative flex shrink-0 overflow-hidden border ${surfaceClasses}`,
      ...
      style={mergedStyle}
    >
      <div className={`playing-card__corner absolute left-1.5 top-1.5 flex flex-col font-black leading-none ${inkClass}`}>
        <span className={rankClass}>{card.rank}</span>
        {centerSuitOnly ? null : <span className={cornerSuitClass}>{suitSymbol}</span>}
      </div>
      <div className={`absolute inset-0 flex items-center justify-center font-black leading-none ${inkClass} ${suitClass}`}>
      ...
      <div
        className={`playing-card__corner absolute bottom-1.5 right-1.5 flex rotate-180 flex-col font-black leading-none ${inkClass}`}
      >
        <span className={rankClass}>{card.rank}</span>
        {centerSuitOnly ? null : <span className={cornerSuitClass}>{suitSymbol}</span>}
      </div>
